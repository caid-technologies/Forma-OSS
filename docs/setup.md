...
- `EXTERNAL_SOURCE_PROVIDER` controls external source research for `workflow=web_research`. Supported values are `firecrawl` and `tavily`; legacy `auto` selects Firecrawl.
...
- `TAVILY_API_KEY` enables Tavily search when `EXTERNAL_SOURCE_PROVIDER=tavily`. `TAVILY_SEARCH_LIMIT`, `TAVILY_SEARCH_DEPTH`, `TAVILY_TIMEOUT_SECONDS`, `TAVILY_INCLUDE_ANSWER`, and `TAVILY_INCLUDE_RAW_CONTENT` tune Tavily behavior.
