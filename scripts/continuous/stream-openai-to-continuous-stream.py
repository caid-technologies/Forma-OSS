...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"[openai-stream] listen=./scripts/continuous/listen-continuous-stream.py "
            f"--stream-id {store.stream_id} --latest",
            flush=True,
        )
        print(
            f"[openai-stream] agents=./scripts/continuous/run-continuous-agents.py --stream-id {store.stream_id}",
            flush=True,
        )
        return 0
