...
Generate a local plan:
    python -m forma_core.fabricator plan --material "cellulose acetate offcuts"

Generate through the configured Forma LLM provider:
    python -m forma_core.fabricator plan --live --provider runpod --material "cellulose acetate offcuts"

Inspect Forma MCP tools:
    python -m forma_core.fabricator mcp-tools
"""
...
from forma_core.llm import build_llm_provider, enforce_production_llm_preflight
...
DEFAULT_MCP_URL = config.get("FORMA_MCP_URL", "http://127.0.0.1:8000/mcp")
...
class DeviceInterfaceNeed(BaseModel):
    ...
    interface_type: Literal["forma_mcp", "manual", "external_api", "file_import", "unknown"]
...
class FabricatorPlan(BaseModel):
    ...
    forma_mcp_handoff: list[dict[str, Any]] = Field(default_factory=list)
...
class FabricatorRun(BaseModel):
    ...
    forma_llm: dict[str, Any] | None = None
    forma_mcp_tools: Any | None = None
    ...
    return FabricatorPlan(
        ...
        candidate_workflows=[
            CandidateWorkflow(
                ...
                next_forma_mcp_action="List available inventory, device, validation, and job-creation tools.",
            )
        ],
        ...
        forma_mcp_handoff=mcp_handoff_requests(),
    )
...
def generate_live_plan(
    ...
    if not plan.forma_mcp_handoff:
        plan.forma_mcp_handoff = mcp_handoff_requests()
    ...
    return LatticeAgentCard(
        ...
        capabilities=[
            ...
            LatticeCapability(
                ...
                actions=["fabricator.card", "forma.lattice.get_agent_card"],
            ),
        ],
...
def add_mcp_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help="Forma MCP endpoint URL (defaults to FORMA_MCP_URL or http://127.0.0.1:8000/mcp).",
    )
...
def build_run(args: argparse.Namespace, question: FabricatorQuestion) -> FabricatorRun:
    ...
    if args.live:
        plan, forma_llm, warnings = generate_live_plan(args, question)
        mode = "live"
    else:
    ...
    lattice_run = LatticeRunRecord.completed(
        ...
        warnings=warnings,
        handoff_actions=plan.forma_mcp_handoff,
    ...
    if args.include_mcp_tools:
        run.forma_mcp_tools = post_mcp_json_rpc(
            args.mcp_url,
            {"jsonrpc": "2.0", "id": "fabricator-tools", "method": "tools/list", "params": {}},
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "mcp-tools":
        ...
        write_json_output({"forma_mcp_tools": payload}, args.output, quiet=args.quiet)
        return 0
