...
- `REDIS_URL`: optional Redis connection URL for cached `/projects` and `/my/projects` responses. Cache failures fall back to the database.
...
- `REDIS_CACHE_PREFIX`: optional Redis key namespace, default `blueprint`.
