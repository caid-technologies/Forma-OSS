...
- Local workspace settings are always encrypted: Supabase-backed environments use `workspace_integration_configs`, while environments without Supabase use an encrypted local file. `BLUEPRINT_WORKSPACE_SECRETS_KEY` may provide a separate workspace key; otherwise `BLUEPRINT_USER_SECRETS_KEY` is used.
