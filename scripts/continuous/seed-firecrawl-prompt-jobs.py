...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"[seed-firecrawl-prompts] listen=./scripts/continuous/listen-continuous-stream.py "
        f"--stream-id {store.stream_id} --latest"
    )
    print(
        f"[seed-firecrawl-prompts] run=./scripts/continuous/run-continuous-llm-jobs.py "
        f"--stream-id {store.stream_id} --model {report.model} --max-jobs {pending_hint}"
    )
