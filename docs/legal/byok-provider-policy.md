# Blueprint BYOK Provider Policy

Last reviewed: July 20, 2026

This policy controls which provider credentials Blueprint Cloud may accept from users. It is a conservative product policy, not a formal legal opinion.

## Provider Registry

| Provider | Hosted BYOK | Local BYOK | Self-hosted BYOK | Cloud rule |
| --- | --- | --- | --- | --- |
| OpenAI | Disabled | Enabled | Enabled | Blueprint Cloud uses a CAID-owned/platform-managed key. Do not accept or store user OpenAI API keys. |
...
| Together AI | Enabled | Enabled | Enabled | Image BYOK requires a project-scoped key dedicated to Blueprint. Legacy or broad account keys are not accepted; model-specific terms must be enforced and model IDs recorded for stored outputs. |
| NVIDIA Build | Disabled | Enabled | Enabled | Do not accept user-supplied NVIDIA Build/API Catalog keys in Blueprint Cloud. Free hosted endpoints are trial/evaluation endpoints; production/customer-facing use requires a paid NVIDIA or authorized-provider agreement and reviewed service credentials. |
...
- Use a saved key only for requests initiated by its owning Blueprint account.
...
- GMI Cloud hosted BYOK is conditional. User-supplied hosted keys require explicit confirmation that third-party server-side key delegation is permitted and that the key is scoped to a dedicated project or organization. Platform-managed GMI keys may be used for Blueprint-managed cloud inference.
- GMI Cloud inference output storage is allowed when Blueprint stores prompt, output, model ID, endpoint type, request ID, token usage, and creation time for service operation under the user's retained ownership. Model-specific commercial-use, distribution, and storage restrictions still require validation.
...
- Together AI hosted image BYOK is enabled only for project-scoped API keys dedicated to Blueprint. Legacy keys and broad account keys are not accepted.
