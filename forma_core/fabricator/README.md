...
Fabricator is Forma Core's fabrication-planning package for predicting how products can be fabricated from primitive inputs across chemistry, biotechnology, material science, and life science domains.
...
```bash
python -m forma_core.fabricator plan
```
...
```bash
python -m forma_core.fabricator card
```

Generate a plan and write a JSON artifact:

```bash
python -m forma_core.fabricator plan --material "cellulose acetate offcuts" --include-mcp-tools --output results/fabricator-plan.json
```

Use a running Forma backend (or set `FORMA_MCP_URL`):

```bash
python -m forma_core.fabricator mcp-tools --mcp-url http://127.0.0.1:8000/mcp
```
